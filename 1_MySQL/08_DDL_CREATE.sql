...
/*
    ...
    * 자료형
	...
	3. 날짜 및 시간
		- DATE : 날짜 저정하는데 사용
        - TIME : 시간 저장하는데 사용
        - DARETIME / TIMESTAMP : 날짜와 시간을 함께 저장
	4. 불리언
...
INSERT INTO member 
	VALUES(2, 'user02', 'pass02', '공재욱', '남', 'NULL','NULL', 'current_data'());
INSERT INTO member VALUES(NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);

/*
	제약조건(CONSRTAINTS)
    - 사용자가 원하는 조건의 데티어만 유지하기 위해서 각 컴럼에 대해 저장될 값에 대한 제약조건 설정
    - 제약조건은 테이터 무결성 보장을 목적으로 한다.
    - 종류 : NOL NULL, UNIQUE, CHECK, PRIMARY KEY, FORELGN 
*/
/*
	NOT NULL
    - 해당 컬럼에 반드시 값이 있어야만 하는 경우
		(즉, 해당 컬럼에 절때 NULL이 들어와서는 안되는 경우)
...
-- >> NOL NULL 제약조건에 위배되어 오류 발생!
    INSERT INTO mem_notnull 
    VALUES(2, 'user02','pass01','손정배', '남', null, null, current_date());
    ...
    /*
        ...
        - 추가/수정 시 기존에 있는 데어터값 중 중복값이 있을 경우 오류 발생
    */
  ...
  */
  ...
  CREATE TABLE mem_check (
    ...
    -- >> check 제약조건으로 not null 조건또한 위배되었을음 확인!
    ...
    /*
		PRIMARY KEY (기본키)
        -  테이블에서 각 행들을 식별하기 위해 서용될 컬럼에 부여하는 제약조건 (식별자 역할)
	ex) 회원번호, 학번, 사원번호, 부서코드, 직급코드, 제품번호, 주문번호, 운송장번호, ...
    - PRIMARY KEY 제약조건을 부여하면 그 컴럼에 자동으로 NOT NULL + UNIQUE 조건이 설정
	- 한 테이블 당 오로지 한 개만 설정
    --> 간혹 복합키 설정도 가능
    */
...
SELECT * FROM mem_pril;

-- 복합키 사용 예시 (어떤 회원이 어떤 상품을 찜했는지에 대한 데이터를 보관하는 테이블)
-- >> 복합키 방식으로 지정해서 쓰는건 가급적으로 추천하진 않아요!
...
INSERT INTO pro_like VALUES(1, 'A', current_date()); /* a + 1 = a1 이런식으로 출력됨 */
...
/*
    ...
    - 다른테이블에 존재하는 값만 들어와야 되는 특정 컬럼에 부여하는 제약조건
    (단, NULL 값은 가 수 있음)
...
*/
      ...
      INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
        ...
        INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
        VALUES('user02', 'pass02','김경주', 1) ;
        -- >> 부모키(parent key)를 찾ㄹ을 수없다는 오류 발생! 
 ...
 SELECT * FROM member;
    ...
    -- mem_grade 테이블에서 code가 2인 등급 삭제
 DELETE FROM mem_geade
 WHERE grade_code = 2;
 -- >> 자식 테이블(member)에 2라는 값을 사용하고 있기 때문에 삭제 X
 
   -- mem_grade 테이블에서 code가 3인 등급 삭제
 DELETE FROM mem_geade
 WHERE grade_code = 3;
...
SELECT * FROM mem_grade;
  ...
  INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
        ...
        -- >> 부모키(parent key)를 찾을 수없다는 오류 발생! 
        
        INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
        VALUES('user03', 'pass03','우현성', 2);
        
        -- mem_grade 테이블에서 grade_code가 2번 데이터 삭제
        DELETE FROM mem_grade
        WHERE grade_code = 2;
