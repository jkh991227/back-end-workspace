...
/* 
	1-2 컬럼 수정 (MODIFY)
    - 자료형 변경 : ALTER TABLE 테이블명 MODIFY 컴럼명 변경할 자료형;
    - 기본값 변경 : ALTER TABLE 테이블명 MODIFY 컴렴명 자료형 DEFAULT 변경할 기본값;

	* 참고 : DEFAULT 삭제
    ALTER TABLE 테이블명 MODIFY 컬럼명 자로형 DEFAULT NULL;
*/
...
ALTER TABLE dept_copy MODIFY dept_id VARCHAR(5);
-- >> 변경하려는 자료형의 크기보다 이미 큰 값이 존재하면 에러 발생!

-- dept_title 컬럼의 자료형은 VARCHAR(40),
-- locatin_id 컬럼의 자료형은 VARCHAR(2),
-- lname 컴럼의 기본값을 미국으로 변경 (자료형 : VARCHAR(20))

ALTER TABLE dept_title
	MODIFY dept_id VARCHAR(40),
	MODIFY dept_id VARCHAR(2),
	MODIFY dept_id VARCHAR(20),DEFAULT '미국';
...
ALTER TABLE dept_copy DROP COLUMN coame;
...
*/
-- detp_copy 테이블에 lname 컬럼명을 location_name으로 변경
ALTER TABLE detp_copy RENAME COLUMN lname TO location_name;

/*
	2. 제약조건 추가 / 삭제
    2-1 제약조건 추가 (ADD)
    - ADD [CONSRTAINT 제약조건명] PRIMARY KEY (컬럼명);
   - ADD [CONSRTAINT 제약조건명] FOREIGN KEY (컬럼명) REFERENCES 테이블명 (컬럼명);
    ADD [CONSRTAINT 제약조건명] UNIQUE (컬럼명);
    -  ADD [CONSRTAINT 제약조건명] CHECK (컬럼명);
    - NOT NULL : MODIFY 컬럼명 [CONSRTAINT 제약조건명] NOT NULL;
*/
...
ALTER TABLE dept_title PRIMARY KEY unique(dept_title);
ALTER TABLE dept_copy DROP PRIMARY 

/*
	2-2 제약조건 삭제
    - NOT NULL : ALTER TABLE 테이블명 MODIFY 컬럼명 NULL;
    - PRIMARY KEY : ALTER TABLE 테이블명 DROP PRIMARY KEY;
    - 나머지 : ALTER TABLE 테이블명 DROP CONSTRAINT 제약 조건명;
*/
ALTER TABLE dept_copy DROP CONSTRAINT dept_title;
ALTER TABLE dept_copy DROP PRIMARY KEY;

-- 제약조건명 확인
SELECT TABLE_SVHEMA, TABLE_NAME, CONSTRAINT_NAME, CONSTTAINT_TYPE
FROM inofrmation_schema.TABLE_CONSRTAINTS
...
/*
	3. 테이블명 변경 (RENAME)
    ALTER TABLE 기본테이블명 RENAME to 변경할테이블명;
*/
...
-- 삭제하고 싶다면 자깃 테이블 먼저 삭제 후 부모 테이블 삭제 가능!
